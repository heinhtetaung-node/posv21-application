	...
	*/
	function get_info($item_id, $transaction_type = 'sale') //Can be sale or receiving
	{
		...
		if($transaction_type == 'receiving')
		{
			...
			if ($supplier_id)
			{
				...
				if($supplier_info->override_default_tax)
				{
					...
					{
						return $this->Tax_class->get_taxes($supplier_info->tax_class_id)->result_array();
					}
		...
		if($item_location_info->override_default_tax)
		{
			...
			{
				return $this->Tax_class->get_taxes($item_location_info->tax_class_id)->result_array();
			}
		...
		if($item_info->override_default_tax)
		{
			...
			{
				return $this->Tax_class->get_taxes($item_info->tax_class_id)->result_array();
			}
