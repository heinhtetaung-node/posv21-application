...
class Item extends CI_Model
{
	...
	*/
	function get_all($limit=10000, $offset=0,$col='item_id',$order='desc')
	{
		...
		$this->db->select('suppliers.company_name as supplier_company_name, location_items.location as location, items.*, item_images.image_id as image_id, GROUP_CONCAT('.$this->db->dbprefix('tags').'.name) as tags, categories.id as category_id,categories.name as category,
	...
	*/
	...
	function search($search, $category_id = false, $limit=20,$offset=0,$column='name',$orderby='asc', $fields = 'all')
	{
		...
		$this->db->select('suppliers.company_name as supplier_company_name,location_items.location as location, item_images.image_id as image_id, GROUP_CONCAT('.$this->db->dbprefix('tags').'.name) as tags,items.*,categories.id as category_id,categories.name as category,
		location_items.quantity as quantity, 
		location_items.reorder_level as location_reorder_level,
		location_items.cost_price as location_cost_price,
		location_items.unit_price as location_unit_price');
