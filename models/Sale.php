...
class Sale extends CI_Model
{
	...
	function get_cash_sales_total_for_shift($shift_start, $shift_end)
  {
	...
	function save($items,$customer_id,$employee_id, $sold_by_employee_id, $comment,$discount_reason,$show_comment_on_receipt, $coupons, $payments, $duedate, $sale_id=false, $suspended = 0, $change_sale_date=false,$balance=0, $store_account_payment = 0)
	{
		...
		log_message('info', 'payment_type '.$payment['payment_type']);
		//TTAT
		if($payment['payment_type'] != "Store Account")		
			$duedate = '';
		log_message('info', 'duedate 2 '.$duedate);
		$sales_data = array(
	...
	//TTAT - 2017-08-27
	function get_store_account_duedate($sale_id)
	{	//echo $sale_id;
		$this->db->select('*', false);
		$this->db->from('store_accounts');
		$this->db->where('sale_id', $sale_id);
	
		$store_account = $this->db->get()->row_array();
		
		return date(get_date_format().' '.get_time_format(), strtotime($store_account['duedate']));
	}
}
?>
