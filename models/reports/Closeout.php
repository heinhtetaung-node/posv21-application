...
class Closeout extends Report
{
	...
	public function getData()
	{
		...
		{
			$return[] = array(anchor('reports/closeout/'.$yesterday.'/'.$yesterday.' 23:59:59','<span class="glyphicon glyphicon-backward"></span> '.lang('common_previous_day'), array('class' => 'pull-left')), anchor('reports/closeout/'.$tomorrow.'/'.$tomorrow.' 23:59:59',lang('common_next_day').' <span class="glyphicon glyphicon-forward"></span>', array('class' => 'pull-right')));
		}
		...
		{
			$return[] = array($this->Category->get_full_path($category_sale_row['category_id']),to_currency($category_recv_row['subtotal']).' ('.lang('reports_items_with_tax').': '.to_currency($category_recv_row['total']).')');
		}
		$return[] = array(' ', ' ');
