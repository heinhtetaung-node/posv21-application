...
class Sales extends Secure_area
{
	...
	{
		$this->sale_lib->set_due_date($this->input->post('duedate'));
	}
	...
	{		
		...
		//TTAT
		if($this->input->post('payment_type') == 'Store Account')
			$this->session->set_userdata('duedate', $this->input->post('duedate'));
		else
			$this->session->set_userdata('duedate', '');
		log_message('info'.$this->input->post('duedate'));
	...
	function complete()
	{
		...
		//TTAT		
		$data['duedate']=$this->sale_lib->get_duedate(); 
		//$data['duedate']= date("Y-m-d h:m:s", strtotime($data['duedate']));	
		//log_message('info', 'duedate '.$this->input->post('duedate'));
		log_message('info', 'duedate 1 '.$data['duedate']);

		//SAVE sale to database
		$sale_id_raw = $this->Sale->save($data['cart'], $customer_id, $employee_id, $sold_by_employee_id, $data['comment'],$data['discount_reason'],$data['show_comment_on_receipt'],$this->sale_lib->get_coupons(),$data['payments'], $data['duedate'], $suspended_change_sale_id, 0, $data['change_sale_date'], $data['balance'], $data['store_account_payment']); 
		...
		{			
			if ($this->sale_lib->get_email_receipt() && !empty($cust_info->email))
			{
				...
				$this->email->from($this->Location->get_info_for_key('email') ? $this->Location->get_info_for_key('email') : 'no-reply@mg.amztechnology.com', $this->config->item('company'));
	...
	function receipt($sale_id)
	{
		...
		$data['duedate'] = $this->Sale->get_store_account_duedate($sale_id); //TTAT
