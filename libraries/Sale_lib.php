...
class Sale_lib
{
	...
	function update_register_cart_data()
	{
		...
		$data['duedate'] = $this->get_duedate(); //TTAT
		log_message('info', 'duedate '.$data['duedate']);
	...
	function get_duedate(){
		...
		return $this->CI->session->userdata('duedate') ? $this->CI->session->userdata('duedate') : '';
	}
	function set_due_date($duedate)
	{
		$this->CI->session->set_userdata('duedate',$duedate);
	}
