		...
		<?php if ($customer_required_check) { ?>
			...
			<?php echo form_open("sales/add_payment",array('id'=>'add_payment_form', 'autocomplete'=> 'off')); ?>
				...
				<div style='float:left; margin-bottom:10px;'>
					Due Date: <div id="duedate" class="input-group date datepicker" >
					...
					<?php echo form_input(array(
						...
						'size'=>'8',
						'class' => 'form-control',
							'value'=> date(get_date_format()." ".get_time_format(), $duedate? strtotime($duedate) : time()),
						)
					);?>       
					</div>
				</div>	
...
?>
	...
	$('.xeditable').on('hidden', function(e, editable) {
	 ...
	 $('.xeditable').on('hidden', function(e, editable) {
		...
		//TTAT
		date_time_picker_field($("#duedate"), JS_DATE_FORMAT + " "+JS_TIME_FORMAT);
		$("#duedate").on("dp.change", function(e) {
			$.post('<?php echo site_url("sales/set_due_date");?>', {duedate: $('#duedate').val()});			
      	});
